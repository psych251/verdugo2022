...
library(tidyverse)
library(dplyr)
cc_data <- read.csv('/Users/austinweideman/Desktop/testdata.csv')
data <- cc_data
# view(data)
```
...
```
```{r}
# finally, we can convert this to long form for later analysis with brms
# we will have one column for conditions, and one for curiosity
# conditions: 1 = choice preferred, 2 = no choice/preferred, 3 = no choice/not preferred
...
for (row in 1:nrow(filtered_participant_trials)) {
  ...
  new_choice_row <- c(participant, trial, 'Choice_Pref', c_cur)
  ...
  if (nc_condition == 'True') {
    new_nc_pref_row <- c(participant, trial, 'No_Choice_Pref', nc_cur)
    filtered_participant_trials_long[nrow(filtered_participant_trials_long)+1,] = new_nc_pref_row
  }
  if (nc_condition == 'False') {
    new_nc_not_pref_row <- c(participant, trial, 'No_Choice_Not_Pref', nc_cur)
...
# calculate average curiosity ratings across all valid participants in all selection conditions
avg_all_c_curiosity <- mean(as.numeric(filtered_participant_trials$C_Curiosity))
avg_all_nc_pref_curiosity <- mean(as.numeric(nc_preferred_trials$NC_Curiosity))
avg_all_nc_not_pref_curiosity <- mean(as.numeric(nc_not_pref_trials$NC_Curiosity))

# average choice/preferred curiosity rating
avg_all_c_curiosity

# average no choice/preferred curiosity rating
avg_all_nc_pref_curiosity

# average no choice/not preferred curiosity rating
...
```{r}
# analysis: run brm with curiosity as dependent variable modeled on conditions
library(brms)

brm.fit <- brm(Curiosity ~ Conditions, data = filtered_participant_trials_long, family = cumulative("logit"))
